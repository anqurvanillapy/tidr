...
updatePats {todo = pvar :: ns} env (NBind _ (Pi farg) fsc) (p :: ps)
  = case argType p of
         Unknown =>
            do defs <- get Ctxt
     ...
     = do defs <- get Ctxt
          ...
          fa_tys <- the (Core (Maybe (NF vars), ArgType vars)) $
              case fty of
                   ...
                   Just (NBind _ (Pi NErased) fsc) =>
                      pure (Just !(fsc defs (toClosure env (Ref Bound n))),
                        Unknown)
                   Just (NBind _ (Pi farg) fsc) =>
                      pure (Just !(fsc defs (toClosure env (Ref Bound n))),
  ...
  where
    ...
    addConG {vars'} {todo'} n tag pargs pats rhs []
        = do cty <- the (Core (NF vars')) $ if n == UN "->"
                      then pure $ NBind (MN "_" 0) (Pi NType) $
                              (\d, a => pure $ NBind (MN "_" 1) (Pi NErased)
                                (\d, a => pure NType))
...
sameType {ns} fn env (p :: xs)
  ...
  where
    ...
    headEq (NBind _ (Pi _) _) (NBind _ (Pi _) _) = True
